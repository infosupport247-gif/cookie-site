...
(function () {
  ...
  function getCleanHTML() {
    ...
    clone.querySelectorAll('script').forEach(el => {
      ...
      const isOwnFile = ALLOWED_SCRIPT_SRC_SUFFIXES.some(a => src.endsWith(a) || src.endsWith('/' + a));
      let isAllowedHost = false;
      try { isAllowedHost = ALLOWED_SCRIPT_HOSTS.includes(new URL(src, location.href).host); } catch (e) {}
      if (!isOwnFile && !isAllowedHost) el.remove();
    });
